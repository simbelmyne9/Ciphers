...
void contatenation_128_384(u8 tweakey[48], u8 flag, uint64_t ctr)
{
	for (int j = 0; j < 16; j++)
	{
		if (ctr < ((uint64_t)1 << 8 * j))
		{
			for (int k = 0; k < j; k++)
			{
				tweakey[47 - k] = ((ctr + 1) & (uint64_t)(0xff) << (8 * k)) >> (8 * k);
			}
		}
		else
		{
	...
	tweakey[32] = (tweakey[32] & (0b00001111)) ^ flag;

}

void contatenation_256_384(u8 tweakey[48], u8 flag, uint64_t ctr)
{
	for (int j = 0; j < 8; j++)
	{
		if (ctr < ((uint64_t)1 << 8 * j))
	...
	tweakey[40] = (tweakey[40] & (0b00001111)) ^ flag;

}

void sTMAC_128_256(u8(*pt)[16], size_t pt_bits_num, u8 key[16], u8 N[8], u8 tag[], size_t tau)
{
...
void sTMAC_128_384(u8(*pt)[16], size_t pt_bits_num, u8 key[16], u8 N[16], u8 tag[], size_t tau, bool print_mode)
{
	int last_block_len = pt_bits_num % 128;
	bool need_padding = last_block_len > 0;
	uint64_t block_num = pt_bits_num / 128 + need_padding;
	...
	u8 tweakey[48] = { 0 };
	for (int i = 0; i < 16; i++)	tweakey[i] = key[i];
	for (int i = 16; i < 16 + 16; i++) tweakey[i] = N[i - 16];

	for (uint64_t i = 0; i < block_num - 1; i++)
	{
		u8 Y[16] = { 0 };

		// create tweakey( N(128) || 2(4) || i(60))
		contatenation_128_384(tweakey, 0b00100000, i);

		aesTweakEncrypt(384, pt[i], tweakey, Y);
	...
	if (need_padding)
	{
		pad(pt[block_num - 1], last_block_len);
		XOR(S, pt[block_num - 1]);
		contatenation_128_384(tweakey, 0b00010000, block_num - 1);
		if (print_mode)
			print_u8(S, 16);
	}
	else
	{
		XOR(S, pt[block_num - 1]);
		contatenation_128_384(tweakey, 0b00000000, block_num - 1);
		if (print_mode)
			print_u8(S, 16);
	}
	aesTweakEncrypt(384, S, tweakey, S);
