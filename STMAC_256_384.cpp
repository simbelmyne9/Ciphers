...
void sTMAC_256_384(u8(*pt)[16], size_t pt_bits_num, u8 key[32], u8 N[8], u8 tag[], size_t tau, bool print_mode)
{
	...
	u8 S[16] = { 0 };
	u8 tweakey[48] = { 0 };
	for (int i = 0; i < 32; i++)	tweakey[i] = key[i];
	for (int i = 32; i < 32 + 8; i++) tweakey[i] = N[i - 16];

	for (uint64_t i = 0; i < block_num - 1; i++)
	{
		u8 Y[16] = { 0 };

		// create tweakey( N(64) || 2(4) || i(124))
		contatenation_256_384(tweakey, 0b00100000, i);
	...
	if (need_padding)
	{
		...
		contatenation_256_384(tweakey, 0b00010000, block_num - 1);
	}
	else
	{
		XOR(S, pt[block_num - 1]);
		contatenation_256_384(tweakey, 0b00000000, block_num - 1);
	}
	if (print_mode)
		print_u8(S, 16);
